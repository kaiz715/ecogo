...
def all_distances(driver_address, all_addresses):
    ...
    for i in range(full_requests + 1):
        ...
        for k in range(j):
            distancestring = gmaps.distance_matrix(
                driver_address, passenger_addresses[i * 25 : i * 25 + j]
            )["rows"][0]["elements"][k]["distance"]["text"]
            if distancestring[-2:] == "km":
                passenger_distances.append(
                    float(distancestring[: len(distancestring) - 3])
                )
            else:
                passenger_distances.append(
                    float(distancestring[: len(distancestring) - 2]) / 1000
                )
    # i basically copied this part bc idk what im doing
    passenger_dict = res = {
        passenger_addresses[i]: passenger_distances[i]
        for i in range(len(passenger_addresses))
    }
    ...
    # sorted_passenger_dict = {k: v for k, v in sorted(passenger_dict.items(), key=lambda item: item[1])}
